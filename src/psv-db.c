...
static void libgen(void* ctx_, yml_db_line yml) {
        ...
        if (yml.lib_name && !yml.exp_name) {
                ...
                for (char** weak = (char*[]){"", "_weak", 0}; *weak; weak++) {
                        printf("%s --defsym GEN_WEAK_EXPORTS=%i %s -o %s%s.o\n", as, **weak ? 1 : 0, ctx->lib, ctx->lib,
*weak); printf("%s cru lib%s_stub%s.a %s%s.o\n", ar, ctx->lib, *weak, ctx->lib, *weak); printf("%s lib%s_stub%s.a\n",
rl, ctx->lib, *weak); printf("%s %s%s.o\n", rm, ctx->lib, *weak);
                }
                printf("%s %s\n", rm, ctx->lib);
        }
}
...
static void dump(void* ctx, yml_db_line yml) {
	if (!yml.exp_name)
		return;
	printf("%i %08X %-25s %08X %-25s %s %08X %s\n", yml.yml_firmware, yml.mod_nid, yml.mod_name, yml.lib_nid, yml.lib_name, yml.lib_kernel ? "k" : "u",
	       yml.exp_nid, yml.exp_name);
}

int main(int argc, char** argv) {
	EXPECT(!isatty(STDIN_FILENO), "%s", USAGE);
