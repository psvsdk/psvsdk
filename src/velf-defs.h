...
This header re-include itself 2 times to generate:
- host struct_t (type*) for a more readable code
- target struct_raw (uint32_t) for easy file writing

# TODO

__attribute__((packed, scalar_storage_order("big-endian")))
So we could have a single declaration
...
typedef struct {
	...
	SCE_PTR(sce_module_exports_t*) export_top; /* Offset to start of export table */
	SCE_PTR(sce_module_exports_t*) export_end; /* Offset to end of export table */
	SCE_PTR(sce_module_imports_t*) import_top; /* Offset to start of import table */
	SCE_PTR(sce_module_imports_t*) import_end; /* Offset to end of import table */
	...
	// Included module_sdk_version export in modinfo
	uint32_t module_sdk_version; /* SDK version */
} SCE_TYPE(sce_modinfo);
...
typedef struct {
	...
	SCE_PTR(const void*) sce_param_libc;    /* SceLibc param pointer */
